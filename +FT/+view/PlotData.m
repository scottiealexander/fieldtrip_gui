...
if ~FT_DATA.done.segment_trials
    FT.tools.DataBrowser(FT_DATA.data.time{1},FT_DATA.data.trial{1},'channel',FT_DATA.data.label);
else
    ...
    if strcmpi(win.res.btn,'view')
        i = win.res.condition;
        FT.tools.DataBrowser(FT_DATA.data{i}.time{1},cat(3,FT_DATA.data{i}.trial{:}),'channel',FT_DATA.data{i}.label);
    end
end
