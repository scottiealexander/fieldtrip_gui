...
c = {{'text','string','Time-lock Event:'},...
     {'listbox','string',strList,'tag','type'};...
     {'text','string','Time Before (s):'},...
     {'edit','size',5,'string','','tag','pre','valfun',@ValidateTimes};...
     {'text','string','Time After (s):'},...
     {'edit','size',5,'string','','tag','post','valfun',@ValidateTimes};...
...
i = 0;
while true
    win = FT.tools.Win(c,'title','Define Trials','grid',false,'focus','pre');
    win.Wait;
    ...
    elseif strcmpi('done',win.res.btn)
        ...
        if isempty(types)
            c = {{'text','string',''},...
                 ...
                 {'pushbutton','string','Done','tag','pre','validate',true},...
                 {'pushbutton','string','Cancel','validate',false}};
        end
    end
end
