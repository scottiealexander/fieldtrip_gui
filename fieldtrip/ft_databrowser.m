...
if hasdata
  ...
  data = ft_checkdata(data, 'datatype', {'raw', 'comp'}, 'feedback', cfg.feedback, 'hassampleinfo', 'yes');
...
elseif strcmp(cfg.colorgroups, 'chantype')
  ...
  ft_logmsg(cfg,'%3d colorgroups were identified\n',length(tmp1))
  R = cfg.channelcolormap(:,1);
  G = cfg.channelcolormap(:,2);
  B = cfg.channelcolormap(:,3);
  chancolors = [R(cfg.colorgroups(:)) G(cfg.colorgroups(:)) B(cfg.colorgroups(:))];
...
ft_logmsg(cfg,'------------------------------------------------------------------------------------\n')
ft_logmsg(cfg,'You can use the following keyboard buttons in the databrowser\n')
ft_logmsg(cfg,'1-9                : select artifact type 1-9\n');
ft_logmsg(cfg,'shift 1-9          : select previous artifact of type 1-9\n');
ft_logmsg(cfg,'                     (does not work with numpad keys)\n');
ft_logmsg(cfg,'control 1-9        : select next artifact of type 1-9\n');
ft_logmsg(cfg,'alt 1-9            : select next artifact of type 1-9\n');
ft_logmsg(cfg,'arrow-left         : previous trial\n');
ft_logmsg(cfg,'arrow-right        : next trial\n');
ft_logmsg(cfg,'shift arrow-up     : increase vertical scaling\n');
ft_logmsg(cfg,'shift arrow-down   : decrease vertical scaling\n');
ft_logmsg(cfg,'shift arrow-left   : increase horizontal scaling\n');
ft_logmsg(cfg,'shift arrow-down   : decrease horizontal scaling\n');
ft_logmsg(cfg,'s                  : toggles between cfg.selectmode options\n');
ft_logmsg(cfg,'q                  : quit\n');
ft_logmsg(cfg,'------------------------------------------------------------------------------------\n')
ft_logmsg(cfg,'\n')
end
...
if isempty(cmenulab)
  ...
  if strcmp(cfg.selectmode, 'markartifact')
    ...
    artval = any(artval,1);
    if any(artval)
      ft_logmsg(cfg,'there is overlap with the active artifact (%s), disabling this artifact\n',opt.artdata.label{opt.ftsel});
      opt.artdata.trial{1}(opt.ftsel, begsel:endsel) = 0;
    else
      ft_logmsg(cfg,'there is no overlap with the active artifact (%s), marking this as a new artifact\n',opt.artdata.label{opt.ftsel});
  ...
  elseif strcmp(cfg.selectmode, 'markpeakevent') || strcmp(cfg.selectmode, 'marktroughevent')
    ...
    if any(intersect(begsel:endsel, [opt.event.sample]))
      ft_logmsg(cfg,'there is overlap with one or more event(s), disabling this/these event(s)\n');
      ind_rem = intersect(begsel:endsel, [opt.event.sample]);
      for iRemove = 1:length(ind_rem)
        opt.event([opt.event.sample]==ind_rem(iRemove)) = [];
      end
    else
      ft_logmsg(cfg,'there is no overlap with any event, adding an event to the peak/trough value\n');
...
switch key
  case {'1' '2' '3' '4' '5' '6' '7' '8' '9'}
    ...
    if opt.ftsel > numart
      ft_logmsg(cfg,'data has no artifact type %i \n', opt.ftsel)
    else
      setappdata(h, 'opt', opt);
      setappdata(h, 'cfg', cfg);
      ft_logmsg(cfg,'switching to the "%s" artifact\n', opt.artdata.label{opt.ftsel});
  ...
  case {'shift+1' 'shift+2' 'shift+3' 'shift+4' 'shift+5' 'shift+6' 'shift+7' 'shift+8' 'shift+9'}
    ...
    if opt.ftsel > numart
      ...
      if isempty(artsam)
        ft_logmsg(cfg,'no earlier "%s" artifact found\n', opt.artdata.label{opt.ftsel});
      else
        ft_logmsg(cfg,'going to previous "%s" artifact\n', opt.artdata.label{opt.ftsel});
  ...
  case {'control+1' 'control+2' 'control+3' 'control+4' 'control+5' 'control+6' 'control+7' 'control+8' 'control+9' 'alt+1' 'alt+2' 'alt+3' 'alt+4' 'alt+5' 'alt+6' 'alt+7' 'alt+8' 'alt+9'}
    ...
    if opt.ftsel > numart
      ...
      if isempty(artsam)
        ft_logmsg(cfg,'no later "%s" artifact found\n', opt.artdata.label{opt.ftsel});
      else
        ft_logmsg(cfg,'going to next "%s" artifact\n', opt.artdata.label{opt.ftsel});
        if opt.trlvis(nearest(opt.trlvis(:,1),artsam),1) < artsam
          arttrl = nearest(opt.trlvis(:,1),artsam);
        else
          arttrl = nearest(opt.trlvis(:,1),artsam)-1;
  ...
  case 'i'
    if strcmp(cfg.viewmode, 'butterfly')
      ...
      ft_logmsg(cfg,'click in the figure to identify the name of the closest channel\n');
      val = ginput(1);
      pos = val(1);
  ...
  case 's'
    ...
    ft_logmsg(cfg,'switching to selectmode = %s\n',cfg.selectmode);
    setappdata(h, 'opt', opt);
    setappdata(h, 'cfg', cfg);
    redraw_cb(h, eventdata);
...
%ft_logmsg(cfg,'redrawing with viewmode %s\n', cfg.viewmode);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ft_logmsg(cfg,'plotting artifacts...\n');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if strcmp(cfg.ploteventlabels , 'colorvalue') && ~isempty(opt.event)
  ...
  ft_logmsg(cfg,eventlabellegend);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if strcmp(cfg.viewmode, 'component')
  ...
  if ~isequal(opt.chanindx, chanindx)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % ft_logmsg(cfg,'plotting component topographies...\n');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
