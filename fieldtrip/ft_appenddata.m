...
for i=1:Ndata
  ...
  ft_logmsg(cfg,'input dataset %d, %d channels, %d trials\n', i, Nchan(i), Ntrial(i));
...
if shuflabel,
  ft_logmsg(cfg,'the channel order in the input-structures is not consistent, reordering\n');
  if prunelabel,
    ft_logmsg(cfg,'not all input-structures contain the same channels, pruning the input prior to concatenating over trials\n');
...
elseif cattrial
  ...
  ft_logmsg(cfg,'concatenating the trials over all datasets\n');
  data = varargin{1};
  data.trial  = {};
  data.time   = {};
...
elseif catlabel
  ...
  ft_logmsg(cfg,'concatenating the channels within each trial\n');
  data = varargin{1};
...
if shuflabel
  ...
  ft_logmsg(cfg,'reordering the channels\n');
