...
if ~isfield(cfg, 'trl') && (~isfield(cfg, 'trialfun') || isempty(cfg.trialfun))
  ...
  ft_warning(cfg,'no trialfun was specified, using ft_trialfun_general');
end

% create the trial definition for this dataset and condition
if isfield(cfg, 'trl')
  % the trial definition is already part of the configuration
  ft_logmsg(cfg,'retaining existing trial definition\n');
  trl = cfg.trl;
  if isfield(cfg, 'event')
    ft_logmsg(cfg,'retaining existing event information\n');
    event = cfg.event;
  else
    event = [];
  end
...
elseif isfield(cfg, 'trialfun')
  ...
  if isempty(cfg.trialfun)
    ...
    ft_logmsg(cfg,'evaluating trialfunction ''%s''\n', func2str(cfg.trialfun));
  end
...
if isfield(cfg, 'trialdef') && isfield(cfg.trialdef, 'eventtype') && isequal(cfg.trialdef.eventtype, '?')
  ...
  ft_logmsg(cfg,'no trials have been defined yet, see FT_DEFINETRIAL for further help\n');
elseif size(trl,1)<1
  error('no trials were defined, see FT_DEFINETRIAL for help');
end

% add the new trials and events to the output configuration
ft_logmsg(cfg,'found %d events\n', length(event));
cfg.event = event;
ft_logmsg(cfg,'created %d trials\n', size(trl,1));
cfg.trl = trl;
