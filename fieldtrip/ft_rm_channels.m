function [cfg] = ft_rm_channels(cfg, data)
% %-----------------------------------------------------------------------%
%
...
%add channels marked as bad to output
if BCHANRM
    cfg.rm_channel = BAD_CHANNELS;
else
    cfg.rm_channel = {};
end

%%% reset the ChanSelect function    
ChanSelect(true);  
...
% ChanSelect
%
% Description: add checkboxes for channel removal next to each channel
%              label
%
% Syntax: ChanSelect([bClose]=false)
%
% In:
%       [bClose] - true if the figure is being close, otherwise false
%
% Out: 
%
% Updated: 2013-07-29
...
if nargin > 0 && ~isempty(varargin{1}) && (islogical(varargin{1}) || isnumeric(varargin{1}))
    bClose = varargin{1};
else
    bClose = false;
end
...
if bClose
    ...
    pAxes([1,3]) = pAxes([1,3])*sizScreen(3);
    pAxes([2,4]) = pAxes([2,4])*sizScreen(4);    
    ...
    cellfun(@(x,y) uicontrol('Style'          , 'checkbox'    ,...
                             ...
                             'KeyPressFcn'    , @keyboard_cb  ,...
                             'CallBack'       , @RmChannel    ...
                             ),cPosBx,cLabel);

     %add the accept button
     uicontrol('Style','pushbutton','String','Remove Channels','Units','normalized',...
               'Position',[.01 .95 .1 .05],'Parent',h,'Callback',@CloseFig,'Tag','chan_sel_bx');

     uicontrol('Style','text','String','Mark for Removal','Units','normalized',...
               'Position',[.01 .92 .1 .02],'FontWeight','bold','FontSize',12,...
               'BackgroundColor',get(h,'Color'),'Tag','chan_sel_bx','Parent',h);
end

    %--------------------------------------------------------------------------%
    function RmChannel(obj,evt)
        str = get(obj,'UserData');
        if get(obj,'Value')
            %channel is marked from removal
            BAD_CHANNELS{end+1,1} = str;
        else
            %channel is un-marked (i.e. do NOT remove)
            BAD_CHANNELS = BAD_CHANNELS(~strcmpi(str,BAD_CHANNELS));
        end
