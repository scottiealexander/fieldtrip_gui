...
if isfield(cfg, 'event')
  ft_logmsg(cfg,'using the events from the configuration structure\n');
  event = cfg.event;
else
  try
    ft_logmsg(cfg,'reading the events from ''%s''\n', cfg.headerfile);
...
if isempty(event)
  ft_logmsg(cfg,'no events were found in the datafile\n');
  return
end
...
if Neventtype==0
  ...
  ft_logmsg(cfg,'the following events were found in the datafile\n');
  for i=1:Neventtype
    ...
    ft_logmsg(cfg,'event type: ''%s'' ', eventtype{i});
    ft_logmsg(cfg,'with event values: %s', eventvalue);
    ft_logmsg(cfg,'\n');
  end
end
...
if Neventtype==0
  ...
  if isempty(strsettings)
    ft_logmsg(cfg,'no events of the selected type were found in the datafile\n');
    return
  end
